...
def test_helper():
    c_file = Path('../../data/chrome_debian/raw_code/19493_0.c')
...
def test_nested_pseudo_blocks():
    # This file has a for loop nested in a for loop nested in an if loop, with pseudo blocks.
    c_file = Path(test_data_root / 'acceptance/misc/chrome_debian_19493_0.c')
    test_transform = LoopExchange
    with open(c_file) as f:
        old_code = f.read()
        old_lines = old_code.splitlines(keepends=True)
